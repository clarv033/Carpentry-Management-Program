...
class CarpentryApp(QMainWindow):
    ...
    def update_completers(self):
        """updates autocomplete for search bars"""
    ...
    def load_customer_data(self):
        ...
        general_search = self.cust_search.text().lower()
        ...
        for row in database.get_customers():
            ...
            if general_search in name_text or general_search in phone_text:
    ...
    def save_to_inventory(self):
        ...
        if material.lower() in registered_mats and quantity.isdigit():
            ...
            material_name = database.get_registry()[idx]
            database.add_inventory(material_name, int(quantity))
    ...
    def remove_stock(self):
        """uses stock, shows error if not enough"""
    ...
    def delete_from_inventory(self):
        """deletes material from inventory"""
        row = self.inventory_table.currentRow()
        if row >= 0:
